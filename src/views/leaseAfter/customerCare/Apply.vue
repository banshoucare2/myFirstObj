<template>
  <div class="page-container" :style="{ minHeight: SCREEN_HEIGHT }">
      <van-sticky :offset-top="0">
         <van-tabs class="vanTabs" v-model="activeName">
            <van-tab title="基本信息" name="0" >
              ...
              />
              <van-field            
                :value="formatMoney((Number(result.changeFee) - Number(result.feeRelief)).toFixed(2) || 0) +'元'"
                label="减免后应收费用(元)"
                readonly
              />
               <!--  -->
            ...
            <van-tab title="附件信息" name="1" >
              <Upload-component  ref="Upload" @saveCallback = 'saveApplyInfoFn' :uploadList="uploadList" :isSave="true"/>          
            </van-tab>
          </van-tabs>
       </van-sticky>
       <van-dialog
        ...
        <van-field
          ...
          <template slot="input">
            ...
            >
              ...
              <van-radio v-if="activityType != 'GD'" name="0">驳回</van-radio>
              <van-radio v-if="task.activityName == '上会投票'" name="2">弃权</van-radio>
            </van-radio-group>
          </template>
        </van-field>
        <van-cell-group >
      ...
      >
        ...
        <div class="dialog-footer">
          <span @click="showAuth1 = false">取消</span>
          <span class="confirm" @click="confirmDialog1">确认</span>
        </div>
      </van-dialog>  
        ...
        <div style="max-height: 300px;overflow-y: scroll">
          <div style="height: 50px;" v-for="(item, index) in task.variables.userList" :key="item.lookup_VALUE">    
            <div style="position: relative;height: 50px;line-height: 50px;">
              ...
              <van-checkbox @click="clickcheckbox" style="position: absolute;right: 10px;top: 15px;" v-model="item.checked"></van-checkbox>
            </div>            
          </div> 
        </div>     
      </van-popup>
...
import { getDetailById, sendSign, sendSms, passKillBill,  getRepayPlan, getChangedRepayPlan, qryIsyqContractNo, saveApplyInfo, upMeeting, isPassMeeting, isAddFn } from '@/service/customerCare'
export default {
  name: 'processForm',
  props: {
    businessKey: {
  ...
  created () {
    ...
    this.$on('GD', function () {
      this.showAuth = true
      this.activityType = 'GD'
    })
    this.$on('RZBFZR', function () {
      this.showAuth = true
    ...
    this.$on('KHJL', function () {
      let this1 = this
      this1.$refs.Upload.saveFn(function(){
        this1.showAuth = true
      })   
    })
   // 上会投票
   this.$on('meetingResult', function () {
      this.showAuth = true
    })
    this.$on('add', function () {
      this.showAuth = true
      this.toupiaoShow = true
      isAddFn({applyId: this.businessKey}).then(res => {
        if (res.code == 0) {
  ...
  methods: {
     async confirmDialog1() {
      ...
      if (response2 && response2.code == "0") {
        this.$toast("审批提交成功");
        this.showAuth1 = false;
        this.$parent.routeToDetail();
      }
    },
    upMeetingFn () {
      if (this.task.activityName == '评审会秘书安排上会' && this.approveForm.agree == 1) {
        let userList = this.task.variables.userList.filter(val => this.selectSomeone.find(a => val.lookup_VALUE_NAME == a)) 
        var obj = userList.map(a => {
          return {
            applyId: this.businessKey,
            applyType: 2,
            taskId: this.$parent.taskId,
            id: a.lookup_VALUE,
            members: a.lookup_VALUE_NAME
          }
        })
        upMeeting(obj).then(res => {
          if (res && res.code == '0') {
            this.$toast('审批提交成功')
            this.showAuth = false
            this.successFn()
    ...
    newSubmint () {
      let agree = this.radio
      let obj = {
        "applyId": this.businessKey,
        "members": this.userInfo.userName,
        "votingOpinions": agree,
        applyType: 2,
        id: this.userInfo.id,
        taskId: this.$parent.taskId,
      }
      isPassMeeting(obj).then(res => {
        if (response2 && response2.code == '0') {
          this.$toast('审批提交成功')
          this.showAuth = false
          this.successFn()
    ...
    saveApplyInfoFn(uploadGD,callback) {
      let this1 = this
      let obj = {
        ...
        if (res.code == 0) {
          ...
          getChangedRepayPlan(obj).then((res1) => {
            if (res1.code == 0) {
              ...
              this1.result.uplodeFileInfo = uploadGD   
              saveApplyInfo(this1.result).then((res2) => {
                if (res2.code == 0) {
                  if (callback) {
                    callback()
                  } else {
    ...
    async confirmDialog () {
      ...
      if (this.task.activityName == '评审会秘书安排上会' && this.radio == 1) {
    ...
    successFn () {
      ...
      if (this.activityType == 'CW') {      
        passKillBill({applyId:this.businessKey}).then((res) => {})
      }
      if (this.activityType == 'RZBFZR' && this.approveForm.agree) {      
        sendSms({applyId:this.businessKey}).then((res) => {})
      }    
    },
    discountReasonFn () {
      if (this.task && this.task.activityId == 'KHJL') {
        this.$refs.Upload.saveFn()
      }
    },
    seachZfb (callback) {
        ...
        if (Number(this.result.changeFee) - Number(this.result.feeRelief) < 0) {
          this.result.feeRelief = 0
          this.$toast('减免后应收费用不能小于0')
          return 
        } 
        this.$refs.Upload.saveFn()
    },
    ...
    // },
    // preview(GDPath) {
    //   GDPath ='https://t8.baidu.com/it/u=2661189947,4292098609&amp;fm=218&amp;app=126&amp;size=f242,150&amp;n=0&amp;f=PNG?s=27E8FE584E61441788193C5F0300D076&amp;sec=1710608400&amp;t=897c55d76c282bec3edde64e7540e0b3'
    //   let url =
    //   BASE_CONFIG.docs_host +
    //     "onlinePreview?url=" +
    //     encodeURIComponent(Base64.encode(GDPath));
