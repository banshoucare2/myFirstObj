    ...
    <div class="creditList">
        <div class="page-container creditlistform" :style="{ minHeight: SCREEN_HEIGHT }">
          <van-sticky style="position: relative;">
            <van-search  v-model="searchKeyWord" @input="onLoad()" placeholder="请输入事项标题" />
            ...
            <van-list :immediate-check="false" v-if="list && list.length > 0" v-model="loading" finished-text="没有更多数据了" :finished="finished" @load="onLoad(2)">
              <div class="list-item" :key="item.fileNo" v-for="item in list">
                ...
                <div class="list-item-content">
                  <div class="item-content-text">事项标题: {{ item.subject }}</div>
                  <div class="item-content-text">状态: <span style="color: red;">{{ statusFn(item.status) }}</span></div>
                  <div class="item-content-text">创建时间: {{ item.createTime }}</div>
                </div>
                <div style="display: flex;justify-content: space-around;">
                  <div :class="['list-item-btn', 'flex-row-center']" @click="routeToDetail(item, true)">
                  办理
                  </div>
...
import { geCamunda, deleteTo} from '@/service/common'
export default {
  name: 'HelloWorld',
  data () {
    return {
  ...
  methods: {
    ...
    async onLoad (type) {
      ...
      const params = { pageIndex: this.pageIndex, pageSize: this.pageSize, subject:this.searchKeyWord}
      const response = await geCamunda(params)
      ...
      if (response && response.code == '0') {
        ...
        let arryPass = ['【收车】发起','延展期报停','兑现批次','金融产品','绩效']
        // this.list = this.list.filter(val => {
        //   if (arryPass.find(a =>val.subject.includes(a))) {
        //     return true
        //   }
        // })
        this.loading = false
    ...
    routeToDetail (item) {
      ...
      // this.$router.push({ name: 'auth', params: { taskId: item.taskId } })
      this.$router.push({ path: '/dingding/auth', query: { taskId: taskId } })
    }
  }
}
</script>
